...
class Solution {
    public int solution(int[] schedules, int[][] timelogs, int startday) {
        ...
        for(int i = 0; i < schedules.length; i++) {
            int sch1 = schedules[i];    // 직원이 설정한 출근 희망 시각
            int sch2 = addMinutes(sch1, 10);    // 출근해야하는 시각
            int timepass = 0;
            ...
            for(int j = 0; j < timelogs[i].length; j++) {
                ...
                if(dayOfWeek == 6 || dayOfWeek == 7) { continue; }    // 주말 건너뛰기
                if(timelogs[i][j] <= sch2) { timepass++; }  // 정상출근 요일 체크
            }
            if(timepass == 5) { answer++; }
        }
        return answer;
    }
